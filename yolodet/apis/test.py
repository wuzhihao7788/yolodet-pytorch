...
from yolodet.models.heads.base import non_max_suppression, clip_coords
from yolodet.models.loss.base import box_iou
from yolodet.models.utils import torch_utils
from yolodet.utils.util import ap_per_class


def single_gpu_test(model, data_loader,half=False, save=False):
    ...
    for batch_i, batch in enumerate(tqdm(data_loader, desc=s)):
        ...
        for si, pred in enumerate(output):
            if save:
                filename = batch['img_metas'][si]['filename']
            ...
            if pred is None:
                if save:
                    cv2.imwrite(os.path.join(eval_result,os.path.basename(filename)), img)
                if nl:
            ...
            if nl:
                ...
                if save:
                    ...
                    for _p in _tb:
                        ...
                        right_bottom = (int(_p[2]), int(_p[3]))
                        cv2.rectangle(
                            img, left_top, right_bottom, color=(255, 255, 0), thickness=2)
                        label_text = str('True')
                        # label_text += '|{:.02f}'.format(_p[4])
                        cv2.putText(img, label_text, (int(_p[0]), int(_p[1]) - 2), cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5,
                                    color=(255, 255, 0))
