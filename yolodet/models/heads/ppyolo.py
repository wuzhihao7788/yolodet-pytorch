...
class Y(nn.Module):
    def __init__(self,
        ...
        self.c_conv3x3 = CoordConv(in_channels=in_channels,
...
class PPHead(BaseHead):
    ...
    def loss(self,
        ...
        indices, tbox, tcls, anchors, ignore_mask = self.get_target(pred=pred,
                                                                    img_metas=img_metas,
        ...
        if self.yolo_loss_type is not None:
            bbox_loss, conf_loss, cls_loss = multi_apply(self.loss_single,
                                                         pred,
                                                         indices,
                                                         tbox,
                                                         tcls,
            ...
            aware_loss, xy_loss, wh_loss = multi_apply(self.loss_single,
                                                       pred,
                                                       indices,
                                                       tbox,
                                                       tcls,
                                                       ...
                                                       ignore_mask)
            return dict(bbox_loss=bbox_loss, conf_loss=conf_loss, cls_loss=cls_loss,
                        aware_loss=aware_loss, xy_loss=xy_loss, wh_loss=wh_loss)
    ...
    def loss_single(self,
        ...
        lcls, lbox, lobj, laware, lxy, lwh = ft([0]).to(device), ft([0]).to(device), \
        ...
        if nb:
            ...
            if self.label_smooth:  # https://arxiv.org/pdf/1812.01187.pdf
                ...
                smooth_onehot = t * (1 - self.deta) + \
                                self.deta * torch.from_numpy(uniform_distribution).to(device)
                t = smooth_onehot
    ...
    def get_eval_bboxes(self, x):
        ...
        for i, mask in enumerate(self.anchor_masks):
            ...
            if self.iou_aware and self.base_num == 6 and self.iou_aware_factor < 1:
                ...
                new_obj = torch.pow(obj, (1 - self.iou_aware_factor)) * \
                          torch.pow(ioua, self.iou_aware_factor)

                new_obj = torch.clamp(new_obj, 1e-7, 1 / 1e-7)
                one = torch.ones_like(new_obj)
                new_obj = torch.clamp((one / new_obj - 1.0), 1e-7, 1 / 1e-7)
