...
class PANet(nn.Module):
    def __init__(self,
        ...
        # out_channels = 512
        self.spp = YOLO_SPP(in_channels[-1],
                            kernel_sizes=kernel_sizes,
                            norm_type=norm_type,
                            num_groups=num_groups)
        # out_channels = 256
        self.upsample1 = UpSampleModule(in_channels[-2],
                                        norm_type=norm_type,
                                        num_groups=num_groups)
        # out_channels = 128
        self.upsample2 = UpSampleModule(in_channels[-3],
                                        norm_type=norm_type,
                                        num_groups=num_groups,
                                        sam=sam)
        self.downsample1 = DownSampleModule(in_channels[-2] // 2,
                                            norm_type=norm_type,
                                            num_groups=num_groups,
                                            sam=sam)
        self.downsample2 = DownSampleModule(in_channels[-3] // 2,
                                            norm_type=norm_type,
                                            num_groups=num_groups,
                                            sam=sam)
