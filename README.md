...
## 预训练模型
由于本人资源有限，只有单张1080Ti的显卡可供训练，完整训练完训练周期较长。为了说明本框架可训练，推断和测试。本次给出的预训练模型只是训练12个epoch的模型，损失值还在下降中，未训练充分，仅供大家参考。如有条件的开发者可完成本次训练，并希望你能把训练好的预训练模型提供出来，供大家使用。我也会在重要位置进行声明和感谢。话不多说直接上链接，查看预训练模型请点击【[这里](docs/MODEL_ZOO_cn.md)】
